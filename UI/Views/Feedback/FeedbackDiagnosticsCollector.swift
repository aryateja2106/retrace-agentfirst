...
extension FeedbackService {
    ...
    private func collectSanitizedSettingsSnapshot() -> [String: String] {
        ...
        settings["inactiveIntervalCaptureEnabled"] = boolString(defaults.object(forKey: "inactiveIntervalCaptureEnabled") as? Bool ?? true)
        settings["inactiveCaptureThresholdSeconds"] = String(format: "%.0f", defaults.object(forKey: "inactiveCaptureThresholdSeconds") as? Double ?? 300)
        settings["inactiveCaptureProbeIntervalSeconds"] = String(format: "%.0f", defaults.object(forKey: "inactiveCaptureProbeIntervalSeconds") as? Double ?? 30)
